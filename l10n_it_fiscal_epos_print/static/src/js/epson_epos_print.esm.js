...
export class EpsonEposPrint {
    ...
    setup(options, sender) {
        ...
        this.order = null;
        this.pos = sender.pos || sender.env.pos;
        this.sender = sender;
    ...
    onReceive(res, tagListNames, addInfo) {
        ...
        if (!res.success) {
            if (this.order !== null) {
                ...
                order_lines_msg = order.get_orderlines()
                    .map(
                        (l) =>
                            `\n${l.get_full_product_name() || ""}, Quantity: ${l.get_quantity() || ""}, Price: ${l.price || ""}, Discount: ${l.discount || ""}`
                    )
                    .join(",");
                order_payment_msg = order.paymentlines
                    .map(
                        (l) =>
                            `\nType: ${l.payment_method ? l.payment_method.name : ""}, Amount: ${l.amount || ""}`
                    )
                    .join(",");
                // Save the order
    ...
    printFiscalReceipt(receipt) {
        const lines = receipt.get_orderlines();
        ...
        lines.forEach((l) => {
            ...
            l.set_fp_data();
            const quantity = l.qty || l.quantity;
        ...
        if (hasRefund) {
            ...
            receipt.paymentlines.forEach((l) => {
                ...
                xml += this.printRecTotal({
                    ...
                    paymentType: l.payment_method.fiscalprinter_payment_type,
                    paymentIndex: l.payment_method.fiscalprinter_payment_index,
                    description: l.payment_method.name,
                    operator: fiscalOperator,
                });
            });
        }
